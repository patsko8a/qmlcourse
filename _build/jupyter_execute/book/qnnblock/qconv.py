...
# # Cверточные вариационные квантовые схемы
...
# Основное отличие по отношению к классической свертке состоит в том, что квантовая схема может генерировать очень сложные ядра, вычисления которых могут быть классически невоиспроизводимыми. Указанные сложные ядра, а именно ядра в высокоразмерном пространстве Гильберта, могут дать преимущества квантовым сверткам по отношению к классическим.
# 
...
def quanv(image):
    ...
    # Циклы по координатам верхего левого пикселя блоков 2х2
...
# Давайте посмотрим на 4 первых тренеровочных примера (первый ряд) и их 3 канала подготовленных данных (2-4 строки)
...
# Обучение будет осуществленно с помощью оптимизатора `Adam` по функции потерь, в качестве которой используется кросс-энтропия.
